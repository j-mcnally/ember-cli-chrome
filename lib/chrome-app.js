...
function injectENVJson(name, fn, env, tree, files) {
  ...
  var envJsonString = function(){
    var my_env = fn(env);
    if (env != "development") {
      my_env.locationType = 'hash'; // Auto & None cause issues in the chrome extension. Hash is safest.
    }
...
EmberCLIChrome.prototype.treeFor = function treeFor(name) {
  // Refactor this to something else later
  if (!fs.existsSync(path.join('app/views/application.coffee')) && !fs.existsSync(path.join('app/views/application.js'))) {
